...
function changeDay() {
  ...
  changeProfit();
  document.getElementById("confirmMessage").innerHTML = "Welcome to Day "+SAM.length;
}
...
function buyConfirm(stockName) {
  ...
  if (shareCost > parseFloat(theBank.textContent)) {
    ...
    window[stockName + "cost"] = parseFloat(window[stockName + "cost"] + shareCost).toFixed(2);
    changeProfit();
    buyDeny()
  }
}
...
function changeSec() {
	//console.log("Whatever, 1, or Hi");
  if (document.getElementById("timeDisplay").textContent == 1) {
...
function init() {
  ...
  setInterval(doWhatIntervalCantDo, 60000);
